...
public class Pivot extends SubsystemBase {
  ...
  private final PIDController pivotUpController = new PIDController(0.16, 0.0, 0.00); // these pid need to be adjusted
  private final PIDController pivotDownController = new PIDController(0.06, 0.0, 0.00);
  
public Command pivotDown() {
  return run(()->
  pivotMotor.set(pivotDownController.calculate(pivotEncoder.getPosition(), -2.8)))//might not be -0.4
  .withName("Pivot Down");
  }

...
public class Pivot extends SubsystemBase {
  ...
  private final PIDController pivotUpController = new PIDController(0.15, 0.0, 0.0); // these pid need to be adjusted
  private final PIDController pivotDownController = new PIDController(0.05, 0.0, 0.0);
  
public Command pivotDown() {
  return run(()->
  pivotMotor.set(pivotDownController.calculate(pivotEncoder.getPosition(), -2.4)))//might not be -0.4
  .withName("Pivot Down");
  }
